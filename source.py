...
class Exploration:
    ...
    def __init__(self, db_name="db_movies"):
...
class SystemRecomendation:
    ...
    def __init__(self, eda=Exploration()):
    ...
    def filter_popularity(self, type, n, yr, m_ten):
        ...
        if type == "Mejor Calificada":
            ...
            if m_ten:
                ...
                query = f'SELECT movieId, title AS Titulo, ROUND(AVG(rating), 2) AS "Promedio calificacion", COUNT(*) AS "Veces vista" FROM full_info GROUP BY movieId HAVING "Veces vista" > 9 ORDER BY "Promedio calificacion" DESC LIMIT {n}'
        
                df = self.eda.create_df(query)
            
                display(df.iloc[:, 1:])
            else:
        
                query = f'SELECT movieId, title AS Titulo, ROUND(AVG(rating), 2) AS "Promedio calificacion", COUNT(*) AS "Veces vista" FROM full_info GROUP BY movieId ORDER BY "Promedio calificacion" DESC LIMIT {n}'
            ...
            query = f'SELECT movieId, title AS Titulo, ROUND(AVG(rating), 2) AS "Promedio calificacion", COUNT(*) AS "Veces vista" FROM full_info GROUP BY movieId ORDER BY "Veces vista" DESC LIMIT {n}'
            ...
            if m_ten:
                ...
                query = f'SELECT released_yr AS "Año de Lanzamiento", title AS Titulo, ROUND(AVG(rating), 2) AS "Promedio calificacion", COUNT(*) "Veces vista" FROM full_info GROUP BY "Año de Lanzamiento", Titulo HAVING "Año de Lanzamiento" = {yr}  AND "Veces vista" > 9 ORDER BY "Año de Lanzamiento" DESC, "Promedio calificacion" DESC'
                ...
                query = f'SELECT released_yr AS "Año de Lanzamiento", title AS Titulo, ROUND(AVG(rating), 2) AS "Promedio calificacion", COUNT(*) "Veces vista" FROM full_info GROUP BY "Año de Lanzamiento", Titulo HAVING "Año de Lanzamiento" = {yr} ORDER BY "Año de Lanzamiento" DESC, "Promedio calificacion" DESC'
    ...
    def popularity_recommendations(self, max_titles=60):
        ...
        wid_1 = widgets.Dropdown(options=["Mejor Calificada", "Mas Vista", "Mejor Calificada x Año de lanzamiento"])
        wid_2 = widgets.BoundedIntText(value=10, min=10, max=max_titles)
        wid_3 = widgets.Dropdown(options=self.eda.create_df("SELECT DISTINCT released_yr FROM full_info").sort_values(by="released_yr").values.flatten().tolist(),
                                disabled=True)
        wid_4 = widgets.Dropdown(options={"Si":True, "No":False}, value=False)
    ...
    def filter_content(self, title, n, fillna_yr=2010):
        ...
        movies = self.eda.create_df(query)
    ...
    def content_recommendations(self, max_titles=60):
        ...
        wid_1 = widgets.Dropdown(options=self.eda.create_df("SELECT DISTINCT title FROM movies ORDER BY title").values.flatten().tolist())
        wid_2 = widgets.BoundedIntText(value=10, min=10, max=max_titles)
    ...
    def filter_knn(self, title, n_neighbors=10):
    ...
    def filter_user_knn(self, user, n_neighbors, fillna_yr=2010):
        ...
        seen_movies = self.eda.create_df(query_s)
    ...
    def knn_user_recommendations(self, max_titles=60):
        ...
        wid_1 = widgets.Dropdown(options=self.eda.create_df("SELECT DISTINCT userId FROM ratings").values.flatten().tolist())
        wid_2 = widgets.BoundedIntText(value=10, min=10, max=max_titles)
    ...
    def filter_colab(self, user_id, n):
    ...
    def colab_recommendations(self, max_titles=60):
        ...
        output = interactive_output(self.filter_colab, {"user_id":wid_1, "n":wid_2})
